    ...
    const fetchEnrollments = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/student/enrollmentspending/${email}`
        );
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          ...
          <div className="mb-6">
            <h2 className="text-xl font-semibold flex items-center gap-2">
              <span className="w-8 h-8 bg-yellow-100 text-yellow-600 rounded-full flex items-center justify-center">
                <Clock className="w-4 h-4" />
              </span>
              Pending Enrollments
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              Courses awaiting instructor or advisor approval
            </p>
          </div>
              ...
              <tbody>
                {enrollments.map((enrollment) => (
                  ...
                  >
                    ...
                    <td className="py-4 text-gray-600">
                      {enrollment.studentId?.department?.departmentName || 'N/A'}
                    </td>
                    <td className="py-4 text-gray-600">
                    ...
                    {/* Status badge */}
                    <td className="py-4">
                      <span className="px-3 py-1 text-sm rounded-full bg-yellow-100 text-yellow-700 font-medium flex items-center gap-2 w-fit">
